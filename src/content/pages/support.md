---
title: "Support & Downloads"
slug: "support"
description: "Migibio technical support and downloads — instructions for use (IFU), standard operating procedures (SOP), videos, and FAQs for veterinary diagnostic equipment and reagents."
---

# Support & Downloads

Migibio provides comprehensive technical documentation and support for our veterinary diagnostic equipment and rapid test reagents.

## Instructions for Use (IFU)

Each Migibio test kit ships with a printed Instructions for Use. Electronic copies are available on request through our technical support team, ensuring your clinic always has the latest version.

## Standard Operating Procedures (SOP)

We provide SOP templates to help veterinary laboratories standardise sample collection, test execution, quality control, and result interpretation across all Migibio analyzers and reagent panels.

## Video Tutorials

Step-by-step video guides cover:

- Analyzer setup and calibration
- ID chip reading and standard-curve loading
- Sample collection and preparation
- Test execution (Standard and Quick modes)
- Result interpretation and printing

## Frequently Asked Questions

**Q: Which analyzers are compatible with Migibio reagents?**  
A: All Migibio canine and feline fluorescence test reagents are compatible with the FIA680 and FIA880 immunofluorescence analyzers.

**Q: How should test kits be stored?**  
A: Store kits at 4–30℃ until the expiration date. Operate tests at 18–28℃ after opening the device, and complete testing within 30 minutes of opening the pouch.

**Q: What sample types are accepted?**  
A: Most kits accept serum or plasma (EDTA anticoagulant recommended). Refer to the specific IFU for each analyte.

**Q: How do I obtain a quote or place an order?**  
A: Contact our sales team (Contact: Martin.Wong) via WhatsApp or email martinwang2024@gmail.com. You may also use the contact form on our Contact Us page.

**Q: What is the difference between antigen and antibody tests?**  
A: Antigen tests detect the pathogen itself and indicate active infection; antibody tests detect the immune response and indicate exposure or immunity. See [Antigen vs. Antibody Testing](/blog/canine-feline-antigen-vs-antibody-testing).

**Q: How do I ensure reliable, reproducible results?**  
A: Follow the IFU precisely, run built-in controls with every test, store reagents correctly, and track lot numbers. For a complete programme, see [Veterinary Laboratory Quality Control](/blog/veterinary-laboratory-quality-control).

**Q: Which analyzer should I choose for my practice?**  
A: The single-channel FIA680 suits low-to-moderate volume, while the multi-channel FIA880 suits high-volume hospitals. See [How to Choose a Veterinary Immunofluorescence Analyzer](/blog/how-to-choose-a-veterinary-immunofluorescence-analyzer).

## Request Documentation

For IFU, SOP, or video access, please contact our support team and include your clinic or company name and the product catalog number.
