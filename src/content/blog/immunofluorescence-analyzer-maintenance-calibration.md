...
category: "blog"
faq: [{"q": "What is the difference between calibration and quality control?", "a": "Calibration sets the measurement curve — it tells the analyzer how a known signal maps to a concentration, using calibration standards. Quality control (QC) then verifies, every day, that the curve is still correct by running a known sample and checking the result lands where it should. Calibration sets the ruler; QC checks the ruler has not warped."}, {"q": "How often should an analyzer be calibrated?", "a": "Follow the manufacturer's schedule, and recalibrate on specific triggers: when you open a new lot of reagent, when QC repeatedly fails, after maintenance or a part change, or when the analyzer reports a drift warning. Routine calibration is typically less frequent than daily QC, but the exact cadence is product-specific."}, {"q": "Why does an analyzer drift?", "a": "Several silent causes: aging of the optical or detection components, dust or residue on the optics, temperature and humidity swings, and reagent-lot variation. Drift is gradual — results stay plausible while slowly moving off-target — which is why daily QC, not a quarterly calibration, is what actually catches it."}, {"q": "What does daily QC involve?", "a": "Run a control sample with a known concentration at the start of each day (or per the product's schedule), plot the result on a control chart, and check it against the expected range. A result that trends outside the range — or shifts consistently in one direction — signals a problem before patient results are affected."}, {"q": "What are the signs an analyzer needs maintenance?", "a": "QC values drifting or failing, an increase in out-of-range or implausible patient results, visible dust or residue, error codes, or any sudden change in performance. A rising coefficient of variation (CV) on repeat runs is an early, objective sign of trouble."}, {"q": "Why does this matter for a veterinary clinic?", "a": "Because every quantitative result — a progesterone timing a breeding, an NT-proBNP triaging a dyspneic cat — rests on the analyzer being accurate that day. A drifted analyzer does not announce itself; it quietly corrupts clinical decisions. Maintenance and QC are what keep a quantitative result trustworthy."}]
---
...
|---|---|---|
| What it does | Sets the curve (signal → concentration) | Verifies the curve is still correct |
| Uses | Calibration standards | Control samples with known values |
| Frequency | On triggers (new lot, maintenance, failed QC) | Daily (or per product schedule) |
| Metaphor | Re-engraving the ruler | Checking the ruler daily |
